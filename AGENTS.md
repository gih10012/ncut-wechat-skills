...
- The owner subsequently requested a separate open-source Linux WeChat CLI with a one-time privileged service installation, allowing ordinary daily CLI reads/sends, and a fix for missing local outgoing messages. This supersedes the earlier daemon deferral for that narrowly scoped CLI helper; it does not activate the unrelated background message receiver. Prepare and validate the CLI/service before the local sudo installation. Prefer the client’s actual message-creation/writeback pipeline over direct database fabrication.
...
- ClawBot must read/send independently over iLink when desktop WeChat is offline. Local history readback is optional acceptance/diagnosis, never a prerequisite or mandatory post-send check. Support media capabilities according to actual protocol evidence; text-only is not the desired final scope.
- ClawBot reads and writes have standing owner authorization; do not request permission again. Personal WeChat writes to filehelper or ClawBot also have standing authorization, including test messages and media. Personal WeChat writes to other targets (including OneBot/native routes) require current verbal authorization or applicable advance authorization covering the target, content and scope; do not ask again within an authorized scope. General integration approval does not authorize arbitrary other recipients.
