...
| v1.b（进行中） | 评估实际支持微信的轻量客户端后端，以 OneBot／MCP 暴露能力 | 原有Linux微信读取及ClawBot已封装并实测MCP；普通好友/群聊OneBot收发优先，仍缺匹配的消息后端。ClawBot桥只算补充：9月19日升级4.1.13后真实收消息、发送回执及Linux本地读回通过，临时桥已退出。企微个人读取仍未接通 |
| 后续业务 | 具体微信链接、企微业务与学校 Web 等价入口 | 学校目录配对及部分身份/入口已验证；深链、完整企微清单、业务等价性和OAuth按实际需求接入 |

不把 HTTP 200、离线测试、UI 截图或已写文档当作真实业务成功；每个能力记录自己的验证范围。真实订场、个人微信身份写入按本人明确或事先授权的范围执行；ClawBot读写已有长期授权。学校夜间失败须区分网络、认证、权限和业务开放时间。
