...
| v1.b（进行中） | 评估实际支持微信的轻量客户端后端，以 OneBot／MCP 暴露能力 | 原有Linux微信读取及ClawBot已封装并实测MCP；9月21日个人身份经OneBot HTTP向文件传输助手发送文字，本人确认仅收到一条，中文、换行和emoji显示正常；同ID防重及内容冲突拒绝也通过。其他对象、媒体、OneBot事件和本地回写未验收。ClawBot补充桥收发与本地读回已通过；企微个人读取仍未接通 |
