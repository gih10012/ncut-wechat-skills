...
- [微信／企微 skill](skills/wechat-personal/SKILL.md)：Linux 微信本地近期消息、会话检索、原生身份向文件传输助手发送固定文字、ClawBot第三方SDK文字/文件/图片收发、语音文件下载及公众号文字层已验证；通用原生发送、OneBot原生发送与企微消息待验收。
...
| v1.b（进行中） | 评估实际支持微信的轻量客户端后端，以 OneBot／MCP 暴露能力 | 原有Linux微信读取及ClawBot已封装并实测MCP；9月21日本人确认手机收到Linux原生身份向文件传输助手发送的固定文字，通用参数路径、原生OneBot、其他对象和媒体仍未验收。ClawBot桥仅为补充：9月19日真实收消息、发送回执及Linux本地读回通过，临时桥已退出。企微个人读取仍未接通 |
...
此前准备的[Android企微通知转存](skills/wechat-personal/references/workflows/android-notifications.md)已按本人最新要求暂缓：手机实际为原生鸿蒙，不再要求安装SmsForwarder。限时接收端、查询命令及MCP只完成了本机测试，没有真实手机投递验收；保留代码但不作为当前待办。OpenHarmony手机接入留待后续，微信完成后再核对无需手机转发的轻量后端，完整企微聊天目标保持未完成。
