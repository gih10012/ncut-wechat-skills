...
本地显示的缺项已缩小到客户端自己的消息生命周期：初始创建 → 客户端 DB 插入/取得本地 ID → 替换发送上下文中的消息对象 → 网络提交 → 回包更新及通知。底层网络发送已获手机收件确认，但此前绕过前半段。2026-09-23 本人 Linux 窗口的一次只读 GDB 观察取得同条消息初始ID 0 → 本地ID 56 → 更新状态2及服务器ID，并记录原生更新参数 `update_type=1, notify=1`；独立 CLI 从本地数据库读到同一ID和服务器ID，本人确认该条在Linux窗口持续显示且仅一条。这是正常窗口发送的实测基线，**不是 CLI 原生发送的修复验收**。实际 `TextMessageHandler` 不走先前误认为通用文字 sender 的断点。静态副本已找到 JSON 文字业务请求汇入该入库链的完整路径，但主动调用的线程、对象及回调生命周期尚未运行验证；不得用直接 SQL 补写或仅刷新界面作为修复。

仅在继续本地显示修复时，`scripts/native_highlevel_probe.py` 可做一次有界、只读的高层请求→入库→本地ID→更新关联观察。它已通过合成 GDB 测试和固定 ELF 签名核对，**尚未真实附加微信**；不要把它或现有低层发送标为本地显示已修复。
