...
2026-09-23已实际验证：wheel 与源码一致；普通用户离线安装后，从仓库外以隔离 Python 模式读取现有账号成功；临时 Unix 服务 health、无权限拒绝、SIGTERM退出及 socket 清理通过。原生调用的合成 GDB 生命周期、读取与安装器共53项测试通过（1项仅CI root场景跳过），公开仓库首轮CI成功。这些证据不覆盖 CAP_SYS_PTRACE 系统服务部署、真实密钥采集或新服务发送。
...
本地显示的缺项已缩小到客户端自己的消息生命周期：初始创建 → 客户端 DB 插入/取得本地 ID → 替换发送上下文中的消息对象 → 网络提交 → 回包更新及通知。底层网络发送已获手机收件确认，但此前绕过前半段。2026-09-23 本人 Linux 窗口的一次只读 GDB 观察取得同条消息初始ID 0 → 本地ID 56 → 更新状态2及服务器ID，并记录原生更新参数 `update_type=1, notify=1`；独立 CLI 从本地数据库读到同一ID和服务器ID，本人确认该条在Linux窗口持续显示且仅一条。这是正常窗口发送的实测基线，**不是 CLI 原生发送的修复验收**。实际 `TextMessageHandler` 不走先前误认为通用文字 sender 的断点。真实高层入口、业务请求及对象/调度约束仍在静态核对；不得用直接 SQL 补写或仅刷新界面作为修复。
