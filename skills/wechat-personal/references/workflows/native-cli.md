...
本地显示的缺项已缩小到客户端自己的消息生命周期：初始创建 → 客户端 DB 插入/取得本地 ID → 替换发送上下文中的消息对象 → 网络提交 → 回包更新及通知。底层网络发送已获手机收件确认，但此前绕过前半段。2026-09-23 本人 Linux 窗口的一次只读 GDB 观察取得同条消息初始ID 0 → 本地ID 56 → 更新状态2及服务器ID，并记录原生更新参数 `update_type=1, notify=1`；实际 `TextMessageHandler` 不走先前误认为通用文字 sender 的断点。真实高层入口、业务请求及对象/调度约束仍在静态核对；不得用直接 SQL 补写或仅刷新界面作为修复。
