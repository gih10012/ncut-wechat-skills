...
状态在`~/.local/state/ncut-wechat-skills/native-send-trial/`，权限0700/0600。固定试验ID禁止重复提交；只有结果明确为调用前的`main_thread_not_idle_in_poll`拒绝、没有worker/arm且调试器已结束，才自动完整归档旧记录后允许再试。其他已有记录均拒绝，结果不确定时先读状态和接收端，不能删除记录后盲重发。`submission_entered`仅代表进入提交，`task_id`仅是客户端任务号，完成回调零错误也仍需接收端看到固定验收文字。`worker_pending`、`callback_pending`不能当成功；`debugger_still_running`会报告活跃PID，需按其实际状态接续，不能重启或强杀未完成的原生调用。
