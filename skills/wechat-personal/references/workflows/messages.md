...
2026-09-21本人已确认手机文件传输助手收到Linux原生主动发送的一次固定文字；此验收仅覆盖该次锁定版本的个人身份文字发送。`native send-status`只读现有试验；`native send --text --request-id`参数路径、OneBot、其他对象及媒体尚未实际验收，见[发送契约](../capabilities/wechat/send-message.md)。此次消息在Linux本地历史未找到，本地回写未接入；不能因此否定手机投递或重发。

剩余缺项是通用原生发送、OneBot原生收发、本地回写、媒体和企微消息。仅当前任务需要时，检查实际客户端、可调用工具或当前接口源码中的局部新证据；旧后台失效只排除该条路径。单项新能力累计探索15分钟即收束，报告证据与缺项，不重跑已有失败枚举。界面只用于登录和观察接口；不开发聊天窗口点击/输入作为业务发送路径，也不为补发送重建读取链、旧伴随端或容器。
...
2026-09-19当时按新版4.1.13复查：本地读取和ClawBot会话读回正常，补充OneBot机器人通道收发通过；原生发送仍缺外部可调用契约。内部`message::send_text_message`字符串及Unix IPC仅为线索，不是已验证API。随后原生入口及固定文字试验已接通，当前范围以本页开头及[原生发送流程](native-send-port.md)为准；历史候选见[OneBot记录](onebot.md)，不重复4.1.1的版本判断。
