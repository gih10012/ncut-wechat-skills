...
description: 按本人微信和企业微信中的具体信息需求，复用已验证的业务网页/API 与公众号文章读取；按需维护接口子流程和登录入口。原生个人聊天接口尚未打通，不能把业务网页会话当成微信/企微通用登录。
---
...
- 原生群消息/私聊：目前返回明确的 `not_connected` 缺项。没有经本人会话验证的消息 API，不自动转截图、旧后台或伴随端搭建；不以公开信息代替本人消息。
