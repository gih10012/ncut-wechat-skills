"""Bounded ClawBot login and inbound reads; no daemon or automatic replies.
...
def request(base, endpoint, *, body=None, token=None, timeout=15):
    ...
    try:
        ...
        raise BotError('BOT_HTTP_' + str(exc.http_status) if exc.http_status >= 400 else 'BOT_BUSINESS_ERROR') from None
    except TimeoutError:
    ...
    if args.operation == 'finish':
        ...
        if status == 'confirmed':
            ...
            if state.get('ilink_bot_id') == new_state['ilink_bot_id']:
                new_state.update({k: state[k] for k in ('cursor', 'pending') if k in state})
...
def main(argv, access):
    ...
    parser.add_argument('operation', choices=['login', 'finish', 'updates'])
